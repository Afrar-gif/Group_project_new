...
/* 1️⃣ EDIT MODE: டேஷ்போர்டில் இருந்து Edit கிளிக் செய்து வந்தால் */
if (isset($_GET['edit_id']) || isset($_GET['id'])) {
    $is_edit = true;
    ...
    if ($row = mysqli_fetch_assoc($res)) {
        ...
        /* 🌟 BUG FIX: டேட்டாபேஸில் 'time' ஃபீல்டில் "08:00 - 12:00" என இருந்தால் அதை உடைத்து தனித்தனி பாக்ஸில் போடுகிறது */
...
if (isset($_POST['submit_alert'])) {
    ...
    /* 🌟 🌟 FIX: தனித்தனி டைம்களை டேட்டாபேஸின் 'time' காலமிற்கு ஏற்றவாறு ஒன்றாக சேர்க்கிறோம் 🌟 🌟 */
    $combined_time = $start_time . " - " . $end_time;

    if ($is_edit) {
        /* 🛠️ UPDATE QUERY: காலம் பெயரை 'time' என்று மாற்றி ஃபிக்ஸ் செய்யப்பட்டுள்ளது */
        ...
        /* 🛠️ INSERT QUERY: Unknown Column எரரைத் தவிர்க்க காலம் பெயர் 'time' என மாற்றப்பட்டுள்ளது */
